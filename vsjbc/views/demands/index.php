...
<form method="GET" class="table-card p-3 mb-3 no-print">
    <div class="row g-2 align-items-end">
        ...
        <div class="col-md-1 d-flex gap-1">
            ...
            <a href="<?= base_url('demandas') ?>" class="btn btn-outline-secondary btn-sm"><i class="bi bi-x"></i></a>
        </div>
    </div>
</form>
    ...
    <div class="table-responsive">
        <table class="table table-hover align-middle mb-0 small">
            <thead class="table-light">
                <tr><th>#</th><th>Título</th><th>Categoria</th><th>Status</th><th>Prioridade</th><th>Responsável</th><th>Prazo</th><th class="no-print">Ações</th></tr>
            </thead>
            <tbody>
            <?php foreach ($demands as $d): ?>
            <tr>
                <td class="text-muted"><?= $d['id'] ?></td>
                <td><a href="<?= base_url('demandas/' . $d['id']) ?>" class="text-decoration-none fw-semibold"><?= esc(truncate($d['title'], 50)) ?></a></td>
                ...
                <td>
                    ...
                    ?>
                    <span class="<?= $late ? 'text-danger fw-semibold' : '' ?>"><?= date_br($d['deadline']) ?></span>
                    <?php else: ?>—<?php endif; ?>
                </td>
                <td class="no-print">
                    <div class="d-flex gap-1">
                        <a href="<?= base_url('demandas/' . $d['id']) ?>" class="btn btn-outline-secondary btn-sm" title="Ver"><i class="bi bi-eye"></i></a>
                        <a href="<?= base_url('demandas/' . $d['id'] . '/editar') ?>" class="btn btn-outline-primary btn-sm" title="Editar"><i class="bi bi-pencil"></i></a>
                        <form method="POST" action="<?= base_url('demandas/' . $d['id'] . '/excluir') ?>" onsubmit="return confirm('Confirmar exclusão?')">
                            <?= CSRF::field() ?>
